  ...
  export function connect(options: Options): Connection {
    ...
    async function open() {
      ...
      ws.onmessage = (event) => {
        ...
        const type =
          typeof json === "object" && json && "type" in json && typeof json.type === "string" ? json.type : undefined
        const id = typeof json === "object" && json && "id" in json && typeof json.id === "string" ? json.id : undefined
        options.log.info("remote-ws received", { bytes: raw.length, type, id })
